...
class Autosave: ObservableObject {
	...
	func write() async {
		...
		do {
			try await text.write(to: getAutosaveUrl(), atomically: false, encoding: .utf8)
		} catch {
	...
	func read(viewContext: NSManagedObjectContext) async {
		do {
			let input = try await String(contentsOf: getAutosaveUrl(), encoding: .utf8)
	...
	func exists() async -> Bool {
		do {
			return try await FileManager.default.fileExists(atPath: getAutosaveUrl().path)
		} catch {
	...
	func delete() async {
		if await exists() {
			do {
				// Delete file
				try await FileManager.default.removeItem(atPath: getAutosaveUrl().path)
			} catch {
